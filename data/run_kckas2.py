...
for pid in pdb_ids:
    try:
        ...
        off = sum(abs(H[i,j]) for i in range(n) for j in range(n) if i != j)
        diag = sum(abs(H[i,i]) for i in range(n))
        F = off / (off + diag) if (off + diag) > 0 else 0
        print(f'{pid:>6s}  {names.get(pid,"?"):>10s}  {n:>3d}  {S:>8.4f}  {F:>8.4f}')
    except Exception as e:
        print(f'{pid:>6s}  {names.get(pid,"?"):>10s}  {"?":>3s}  FAIL: {str(e)[:40]}')
