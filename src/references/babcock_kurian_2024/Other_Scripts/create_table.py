...
def get_dipole_vectors(jffAtoms):
    ...
    for v1, v2, v_to_rot in zip(v1s, v2s, vs_to_rot):
        ...
        dipole_vectors.append(rot_for_dipole.apply(v_to_rot))
    
    return np.array(dipole_vectors)


def main():
    
    parser = argparse.ArgumentParser()
    parser.add_argument("-o", help="Name of output position-dipole file")
    parser.add_argument("-p", nargs='+', help="Space-separated list of PDB files to scan")
    args = parser.parse_args()

    with open(args.o, 'x') as f:
        for fname in args.p:
            atoms = read_pdb(fname)
            ...
            atoms_strings = [f"{pos[0]:.8f} {pos[1]:.8f} {pos[2]:.8f} {vec[0]:.8f} {vec[1]:.8f} {vec[2]:.8f}\n" for pos, vec in zip(atoms_mp_pos, dipoles)]
